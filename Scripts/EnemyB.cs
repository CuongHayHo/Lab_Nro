...
public class EnemyB : MonoBehaviour
{
    ...
    private void CheckScreenBordersAndWrap()
    {
        ...
        // 3. Chạm vào BIÊN TRÊN (Trần) -> VA ĐÁNH BẬT TRỞ LẠI!
        if (pos.y >= BackgroundManager.CeilingY)
        {
            pos.y = BackgroundManager.CeilingY;
            transform.position = pos;
            if (currentWaypoint.y >= pos.y - 0.5f)
            {
                currentWaypoint.y = Random.Range(minSpawnY, (minSpawnY + maxSpawnY) * 0.5f);
                randomTimer = 0f;
            }
        }

        // 4. Chạm vào BIÊN DƯỚI (Địa hình diahinh.png) -> VA ĐÁNH BẬT TRỞ LẠI!
        if (pos.y <= BackgroundManager.GroundSurfaceY + 0.35f)
        {
            pos.y = BackgroundManager.GroundSurfaceY + 0.35f;
            transform.position = pos;
            if (currentWaypoint.y <= pos.y + 0.5f)
            {
                currentWaypoint.y = Random.Range((minSpawnY + maxSpawnY) * 0.5f, maxSpawnY);
                randomTimer = 0f;
            }
        }
    }
